...
if __name__ == '__main__':
    with app.app_context():
        ...
        # Сначала базовые таблицы без внешних ключей
        User.__table__.create(db.engine)
        print("✓ Таблица users создана")

        # Затем таблицы, которые ссылаются на User
        UserBalance.__table__.create(db.engine)
        print("✓ Таблица user_balances создана")

        Match.__table__.create(db.engine)
        print("✓ Таблица matches создана")

        # Затем таблицы, которые ссылаются на обе
        Bet.__table__.create(db.engine)
        print("✓ Таблица bets создана")

        # Остальные таблицы
        RPLTable.__table__.create(db.engine)
        ClubTest.__table__.create(db.engine)
        TestResult.__table__.create(db.engine)
        ...
        if RPLTable.query.count() == 0:
            for i, club in enumerate(RPL_CLUBS, 1):
                ...
                db.session.add(team)
            db.session.commit()
            print("✓ Таблица РПЛ заполнена")

        # Создаем балансы для всех пользователей
        users = User.query.all()
        for user in users:
            user_balance = UserBalance(user_id=user.id, balance=100)
            db.session.add(user_balance)
        db.session.commit()
        print(f"✓ Балансы созданы для {len(users)} пользователей")

        # Тестовые данные для матчей
